/**
 * The media/asset edge proxy: byte-range media, HLS manifest rewriting, and
 * disk-cached images, fetched from the configured upstream.
 *
 * Extracted from `app/invidious/[[...path]]/route.ts`, which was 601 lines doing
 * five unrelated jobs and was named after an upstream while being a generic
 * proxy. The routes that mount this (`/stream`, `/image`, and the legacy
 * `/invidious` alias) are now thin; the upstream is a config detail rather than
 * a URL prefix. See Phase 5 in docs/INVIDIOUS-BOUNDARY-PLAN.md.
 */
...
/**
 * Path prefixes this proxy is mounted under. `/invidious/` is the legacy one,
 * kept working while cached manifests still reference it (Phase 5).
 */
export const STREAM_PROXY_PREFIX = "/stream/";
export const IMAGE_PROXY_PREFIX = "/image/";
export const LEGACY_PROXY_PREFIX = "/invidious/";
