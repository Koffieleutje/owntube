...
export function buildWatchPlayback(
  ...
  if (options?.shorts) {
    // Reliable path first: OwnTube's generated byte-range HLS re-fetches fresh
    // AVC streams server-side, so it sidesteps Invidious's IP-locked googlevideo
    ...
    // iOS, or no adaptive streams: fall back to a native HLS URL when present.
