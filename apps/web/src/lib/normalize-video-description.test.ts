...
import { normalizeUpstreamDescription } from "./normalize-video-description";

describe("normalizeUpstreamDescription", () => {
  it("converts br tags to newlines", () => {
    expect(
      normalizeUpstreamDescription("Line one<br>Line two<br/>Line three"),
    ).toBe("Line one\nLine two\nLine three");
  });
  ...
  it("extracts href from anchor tags", () => {
    ...
    expect(normalizeUpstreamDescription(input)).toBe(
  ...
  it("decodes HTML entities", () => {
    expect(normalizeUpstreamDescription("Tom &amp; Jerry &mdash; fun")).toBe(
      "Tom & Jerry &mdash; fun",
    );
  });

  it("strips remaining tags", () => {
    expect(normalizeUpstreamDescription("<b>Bold</b> text")).toBe("Bold text");
  });
  ...
  it("builds chapter lines from t= deep links when label is plain text", () => {
    ...
    expect(normalizeUpstreamDescription(input)).toBe("0:00 Intro\n2:05 Outro");
  });
